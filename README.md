...
- 管理员主题设置面板，支持背景、布局等站点配置的实时预览、Hub 保存、旧配置导入导出和恢复默认。
...
迁移旧配置：管理员可以点击「导入旧浏览器配置」，或者通过「导入配置」选择以前导出的 `glass-config.json`（也支持直接的配置对象），检查预览后再保存到站点。`public/glass-config.json` 保留为旧格式兼容文件，不再自动加载；以前直接修改该文件的站点应保留文件并通过导入完成迁移。旧本机主题配置不会自动覆盖 Hub 设置。访客自己的深浅色、卡片/列表选择和收藏仍只保存在本机。
