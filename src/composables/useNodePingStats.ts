...
interface PingRecord {
  monitor_window_loss?: number
  monitor_bucket_loss?: number
  client: string
  task_id: number
  time: string
  value: number
...
function buildPingHistory(records: PingRecord[], metricLossPoints?: MetricLossPoint[]): NodePingHistoryPoint[] {
  ...
  for (let index = 0; index < bucketCount; index++) {
    ...
    let totalCount = 0
    let lostCount = 0
    let monitorLoss: number | undefined
    let latencySum = 0
    let latencyCount = 0
    let metricLossSum = 0
    ...
    while (recordIndex < sortedRecords.length) {
      ...
      if (record.timestamp >= startTime) {
        totalCount += 1
        if (record.monitor_bucket_loss !== undefined) monitorLoss = record.monitor_bucket_loss
        if (record.value >= 0) {
    ...
    history.push({
      ...
      loss: metricLossPoints
        ...
        : (monitorLoss ?? (totalCount ? lostCount / totalCount * 100 : null)),
    })
  }
...
function buildStats(records: PingRecord[], metricStats?: PingMetricTaskStats[], metricLossPoints?: MetricLossPoint[]): NodePingStatsState {
  ...
  for (const recordsByTask of taskRecords.values()) {
    ...
    taskLossValues.push(recordsByTask[0]?.monitor_window_loss ?? ((recordsByTask.length - validValues.length) / recordsByTask.length * 100))
