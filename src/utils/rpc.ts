...
export class RpcClient {
  ...
  async call<T>(method: string, params?: Record<string, unknown> | unknown[], signal?: AbortSignal): Promise<T> {
    ...
    try { return await dispatch(method, (params ?? {}) as Record<string, unknown>) as T }
    catch (error) {
      if (error instanceof Error && error.message.startsWith('Monitor 不提供')) throw new RpcError(-32601, error.message)
      throw error
    }
  }
