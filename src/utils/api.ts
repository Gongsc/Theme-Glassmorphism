...
export class KomariApi {
  ...
  async getMe(): Promise<MeInfo> {
    const me = await request<{ authed: boolean; public_page: boolean }>('/me')
    if (!me.public_page && !me.authed) location.assign('/admin/')
    return { logged_in: me.authed, username: me.authed ? '管理员' : '' }
  }
  ...
  async getLoadRecords(uuid: string, hours: number, maxCount?: number, signal?: AbortSignal): Promise<LoadRecordsResponse> {
    return await records({ uuid, hours, max_count: maxCount }) as unknown as LoadRecordsResponse
  }
  ...
  async getPingRecords(uuid: string, hours: number, signal?: AbortSignal): Promise<PingRecordsResponse> {
    return await records({ uuid, hours }, true) as unknown as PingRecordsResponse
  }
}
